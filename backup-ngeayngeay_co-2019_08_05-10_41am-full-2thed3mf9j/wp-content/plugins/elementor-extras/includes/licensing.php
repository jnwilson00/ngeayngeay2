...
class Namogo_Licensing {
	 ...
	 * @since 2.1.0
	 */
	private function print_admin_notice( $title, $description, $action = null, $link = null ) {

		if ( null !== $action ) {
			$action = ( null !== $link ) ? sprintf( $action, $link ) : $action;
		}

		?><div class="ee-admin-notice notice notice-error">
	...
	public function admin_notices() {
		...
		if ( isset( $errors[ $license_data['license'] ] ) ) {
			...
			$this->print_admin_notice( $error_data['title'], $error_data['message'], $error_data['action'], $error_data['link'] );
			return;
		}
