...
$backup_category_view              = $woocommerce_loop['category-view'];
$woocommerce_loop['category-view'] = 'category-pos-' . explode( '-', $text_position )[0] . ( isset( explode( '-', $text_position )[1] ) ? ' category-text-' . explode( '-', $text_position )[1] : '' ) . ( 'light' != $text_color ? ' category-color-' . $text_color : '' );
...
if ( isset( $backup_category_view ) && $backup_category_view ) {
	$woocommerce_loop['category-view'] = $backup_category_view;
} else {
	unset( $woocommerce_loop['category-view'] );
}
if ( 'creative' == $view ) {
