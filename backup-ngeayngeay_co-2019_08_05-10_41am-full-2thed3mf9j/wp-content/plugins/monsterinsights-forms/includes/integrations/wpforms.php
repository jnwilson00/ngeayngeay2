...
class MonsterInsights_Forms_Integration_WPForms {
	...
	public function __construct() {
		...
		add_filter( 'wpforms_entries_list_filters', array( $this, 'add_to_filters' ) );
		...
		add_filter( 'wpforms_export_ai_fields', array( $this, 'add_utm_to_export_options' ) );
		// Add UTM data in the export data.
		add_filter( 'wpforms_export_ai_value', array( $this, 'add_utm_to_export_data' ), 10, 3 );
	}
		...
		if ( ! empty( $entry_id ) && ! empty( $form_data['id'] ) ) {
			if ( ! empty( $_GET['utm_source'] ) ) {
				wpforms()->entry_meta->add( array(
					...
					'data'     => sanitize_text_field( wp_unslash( $_GET['utm_source'] ) ),
				), 'entry_meta' );
			}
			...
			if ( ! empty( $_GET['utm_medium'] ) ) {
				wpforms()->entry_meta->add( array(
					...
					'data'     => sanitize_text_field( wp_unslash( $_GET['utm_medium'] ) ),
				), 'entry_meta' );
			}
			...
			if ( ! empty( $_GET['utm_campaign'] ) ) {
				wpforms()->entry_meta->add( array(
					...
					'data'     => sanitize_text_field( wp_unslash( $_GET['utm_campaign'] ) ),
				), 'entry_meta' );
			}
			...
			if ( ! empty( $_GET['utm_term'] ) ) {
				wpforms()->entry_meta->add( array(
					...
					'data'     => sanitize_text_field( wp_unslash( $_GET['utm_term'] ) ),
				), 'entry_meta' );
			}
			...
			if ( ! empty( $_GET['utm_content'] ) ) {
				wpforms()->entry_meta->add( array(
					...
					'data'     => sanitize_text_field( wp_unslash( $_GET['utm_content'] ) ),
				), 'entry_meta' );
			}
		}
