...
/* fontawesome 5 */
add_action( 'init', 'porto_vc_update_font_awesomeicons' );
	...
	function porto_creative_grid_layout( $layout ) {
		if ( '1' == $layout ) {
			return array(
				...
				array( 'height' => '1-2', 'width' => '1-4', 'width_md' => '1-2', 'size' => 'shop_catalog' ),
...
if ( ! function_exists( 'porto_creative_grid_style' ) ) :
	function porto_creative_grid_style( $layout, $grid_height, $id, $spacing = false, $include_style = true ) {
		...
		foreach ( $layout as $grid ) {
			...
			if ( ! in_array( $grid['height'], $heights ) ) {
				$heights[] = $grid['height'];
			}
		...
		foreach ( $widths as $width ) {
			...
			echo '#' . $id . ' .grid-col-' . esc_html( $width ) . '{ max-width: ' . ( floor( $width_number * 1000000 ) / 10000 ) . '%; }';
		}
		foreach ( $heights as $height ) {
			...
			echo '#' . $id . ' .grid-height-' . $height . '{ height: ' . round( $height_number ) . 'px; }';
		}
		if ( $has_lg_grid ) {
			$widths_lg = array();
			...
			foreach ( $layout as $grid ) {
				if ( ! in_array( $grid['width_lg'], $widths_lg ) ) {
					...
					echo '#' . $id . ' .grid-col-lg-' . esc_html( $grid['width_lg'] ) . '{ max-width: ' . ( floor( $width_number * 1000000 ) / 10000 ) . '%; }';
		...
		foreach ( $widths as $width ) {
			...
			echo '#' . $id . ' .grid-col-md-' . esc_html( $width[1] ) . '{ max-width: ' . ( floor( $width_number * 1000000 ) / 10000 ) . '%; }';
		}
		foreach ( $heights as $height ) {
			$height_arr = explode( '-', $height );
			if ( count( $height_arr ) > 1 ) {
			...
			echo '#' . $id . ' .grid-height-' . $height . '{ height: ' . round( $height_number / 1.5 ) . 'px; }';
		}
		echo '}';
		echo '@media (max-width: 480px) {';
			echo '#' . $id . ' .product-col { max-width: 100%; }';
		echo '}';
		if ( $spacing ) {
			echo '#' . $id . ' .grid-creative { margin-left: -' . ( (int) $spacing / 2 ) . 'px; margin-right: -' . ( (int) $spacing / 2 ) . 'px; }';
			echo '#' . $id . ' .product-col { padding: 0 ' . ( (int) $spacing / 2 ) . 'px ' . ( (int) $spacing ) . 'px; }';
		}
		if ( $include_style ) {
			echo '</style>';
		}
