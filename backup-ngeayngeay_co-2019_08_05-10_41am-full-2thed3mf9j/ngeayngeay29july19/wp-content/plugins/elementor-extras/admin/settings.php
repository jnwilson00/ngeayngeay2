...
class Settings extends Settings_Page {
	...
	*/
	...
	public function get_settings_sections() {
		...
		$license_key = Namogo_Licensing::get_license_key();

		if ( ! empty( $license_key ) ) {
			$license_data = Namogo_Licensing::get_license_data();

			if ( ! empty( $license_data['license'] ) && is_string( $license_data['license'] ) ) {
				if ( Namogo_Licensing::STATUS_VALID === $license_data['license'] ) {
					$license_status = 'active';
				} elseif ( 'http_error' === $license_data['license'] ) {
					$license_status = 'failed';
				} else {
					$errors = Namogo_Licensing::get_status_errors();
