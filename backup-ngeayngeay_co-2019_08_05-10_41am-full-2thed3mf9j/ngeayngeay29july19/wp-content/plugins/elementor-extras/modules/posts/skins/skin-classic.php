...
class Skin_Classic extends Skin_Base {
	...
	public function render_pagination() {
		...
		if ( $has_numbers ) {
			...
			$handle_multiple = 'yes' === $this->get_instance_value( 'pagination_multiple' ) && '' === $this->get_instance_value('infinite_scroll');
			$format = $handle_multiple ? 'page/%#%?posts=' . $this->parent->get_id() : 'page/%#%';
			$format_classic = $handle_multiple ? '?page=%#%&posts=' . $this->parent->get_id() : '?page=%#%';

			$paginate_args[ 'format' ] = $handle_multiple ? $format : user_trailingslashit( $format, 'single_paged' );
			...
			if ( is_singular() && ! is_front_page() ) {
				...
				if ( ! $wp_rewrite->using_permalinks() ) {
					...
					$paginate_args['format'] 	= $handle_multiple ? $format_classic : user_trailingslashit( $format_classic, 'single_paged' );
				} else {
					$paginate_args['format'] 	= user_trailingslashit( $format, 'single_paged' );
				}
			}
		...
		?><nav <?php echo $this->parent->get_render_attribute_string( 'pagination' ); ?>><?php
			if ( $has_prev_link ) $this->parent->render_previous_nav_link();
			if ( $has_numbers ) echo $pagination;
			if ( $has_next_link ) $this->parent->render_next_nav_link();
		?></nav><?php
	}	
