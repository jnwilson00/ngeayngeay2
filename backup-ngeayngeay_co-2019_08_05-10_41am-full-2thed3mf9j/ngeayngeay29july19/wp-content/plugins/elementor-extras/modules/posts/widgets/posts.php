...
class Posts extends Extras_Widget {
	...
	public function set_filters( $taxonomy = null ) {
		...
		if ( 'yes' === $this->get_skin_setting( 'classic_filters_show_all' ) && 'yes' === $this->get_skin_setting( 'infinite_scroll' ) ) {
	...
	public function render_previous_nav_link( $limit = null ) {
		...
		if ( $page > 1 ) {
			...
			printf( '<a class="ee-pagination__previous page-numbers" href="%1$s">%2$s</a>', get_pagenum_link( $prev_page ), $this->get_skin_setting( 'pagination_previous_label' ) );
		}
	}
	...
	public function render_next_nav_link( $limit = null ) {
		...
		if ( $next_page <= $limit ) {
			printf( '<a class="ee-pagination__next page-numbers" href="%1$s">%2$s</a>', get_pagenum_link( $next_page ), $this->get_skin_setting( 'pagination_next_label' ) );
		}
	}
	...
	public function set_query( $query_args ) {
		...
		add_filter( 'elementor/query/get_query_args/current_query', [ $this, 'override_current_posts_per_page' ] );

		$this->_query = $elementor_query->get_query( $this, 'posts', $query_args, [] );

		remove_filter( 'elementor/query/get_query_args/current_query', [ $this, 'override_current_posts_per_page' ] );
	}
	 ...
	 */
	public function override_current_posts_per_page( $global_args ) {

		$posts_per_page = $this->get_settings( 'posts_per_page' );

		if ( $posts_per_page && $posts_per_page > 0 ) {
			return array_merge( $global_args, [ 'posts_per_page' => $posts_per_page ] );
		}
